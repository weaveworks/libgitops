...
func (p *patcher) Create(new runtime.Object, applyFn func(runtime.Object) error) (patchBytes []byte, err error) {
	...
	if err = encoder.Encode(frame.NewJSONWriter(content.NewWriter(&oldBytes)), old); err != nil {
		return
	}
	...
	if err = encoder.Encode(frame.NewJSONWriter(content.NewWriter(&newBytes)), new); err != nil {
		return
	}
...
func (p *patcher) serializerEncode(input []byte) ([]byte, error) {
	obj, err := p.serializer.Decoder().Decode(frame.NewJSONReader(content.FromBytes(input)))
	if err != nil {
		return nil, err
	}

	var result bytes.Buffer
	if err := p.serializer.Encoder().Encode(frame.NewJSONWriter(content.NewWriter(&result)), obj); err != nil {
		return nil, err
	}
